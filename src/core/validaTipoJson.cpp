...
void validadorTipo::valida_fluidos_producao(Value &fluidos_producao_json, std::vector<std::string> &erros, bool &sucesso) {
    ...
    for (SizeType i = 0; i < fluidos_producao_json.Size(); i++) {
        ...
        // Validar campo modeloCondBlackOil (opcional)
        if (fluidos_producao_json[i].HasMember("modeloCondBlackOil")) {
            if (!fluidos_producao_json[i]["modeloCondBlackOil"].IsInt()) {
                sucesso = false;
                erros.push_back(indiceFluido + "/modeloCondBlackOil: deve ser do tipo inteiro");
            }
        }
...
void validadorTipo::valida_parafina(Value &parafina_json, std::vector<std::string> &erros, bool &sucesso) {
    ...
    if (parafina_json.HasMember("TIACusuario")) {
        if (!parafina_json["TIACusuario"].IsNumber()) {
            sucesso = false;
            erros.push_back("parafina/TIACusuario: deve ser do tipo number");
        }
    }

    if (parafina_json.HasMember("TIACusuarioAtiva")) {
        if (!parafina_json["TIACusuarioAtiva"].IsBool()) {
            sucesso = false;
            erros.push_back("parafina/TIACusuarioAtiva: deve ser do tipo booleano");
        }
    }
...
void validadorTipo::valida_perfil_producao(Value &perfilProducao_json, std::vector<std::string> &erros, bool &sucesso) {
    ...
    std::vector<std::string> camposBooleanos = {
        ...
        "calor", "masstrans", "cpgas", "cpliq", "condgas", "condliq", "condoleo","QLstd", "QLWstd", "QLstdTotal", "QGstd",
